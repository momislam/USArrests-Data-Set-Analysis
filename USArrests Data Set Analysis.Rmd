...
This analysis is going to determine whether a state in the USA is dangerous regarding rape, murder and assault,as well as give an warning message if the state name of the user's input is invalid. The USArrests data set is already in **RStudio** built-in system, therefore, there is no need to import data from external sources.
...
```
### Creating a function to determine wheather a state is dangerous regarding rape, murder and assault,as well as settint the warning message if the state name of the user's input is invald.
